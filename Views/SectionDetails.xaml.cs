...
namespace Winton.Views
{
    ...
    public partial class SectionDetails : Window
    {
        ...
        public async void LoadProducts(string sectionID)
        {
            try
            {
                var products = await Task.Run(() => ProductPlacementServices.GetProductsBySectionAsync(sectionID));
                Dispatcher.Invoke(() =>
                {
        ...
        private async void RemoveProduct_Click(object sender, RoutedEventArgs e)
        {
            if (lstProducts.SelectedItem != null)
            {
                ...
                if (selectedProduct != null)
                {
                    ...
                    await ProductPlacementServices.ArchiveProductPlacementAsync(ProductID, selectedProduct.SectionID, removalNotes, quantitySold, revenue);
